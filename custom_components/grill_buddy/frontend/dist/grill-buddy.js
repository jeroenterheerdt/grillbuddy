...
/**
    ...
    the bug.
  `)}function Le(e){return e.units==Be?"°C":"°F"}const Ne=u`
  ha-card {
    display: flex;
...
`;let Ie=class extends(Oe(he)){hassSubscribe(){return this._fetchData(),[this.hass.connection.subscribeMessage((()=>this._fetchData()),{type:Ee+"_config_updated"})]}async _fetchData(){this.hass&&(this.config=await Pe(this.hass),this.data=this.config)}firstUpdated(){(async()=>{await _e()})()}render(){return this.hass&&this.config&&this.data?z`<h1>Barf</h1>`:z``}saveData(e){var t,r;this.hass&&this.data&&(this.data=Object.assign(Object.assign({},this.data),e),(t=this.hass,r=this.data,t.callApi("POST",Ee+"/config",r)).catch((e=>Ce(e,this.shadowRoot.querySelector("ha-card")))).then())}toggleInformation(e){var t;const r=null===(t=this.shadowRoot)||void 0===t?void 0:t.querySelector("#"+e);r&&("hidden"!=r.className?r.className="hidden":r.className="information")}static get styles(){return u`
      ${Ne}
      .hidden {
        display: none;
      }
...
<div class="probeline">
      ...
      `:z``}renderTheSourceOptions(e,t){if(this.hass){let r=z`<option value="" ?selected=${void 0===t}">---${fr("common.labels.select",this.hass.language)}---</option>`;return Object.entries(e).map((([e,i])=>r=z`${r}
            <option
              value="${i.name}"
              ?selected="${t===i.name}"
            >
